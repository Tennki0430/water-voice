...
- 🛡 **完全オフライン動作** — 音声・テキストを外部サーバーに送信しません（ネットワーク通信なし）
- 🛡 **外部ライブラリ依存ゼロ** — Apple 純正フレームワークのみを使用
- ⚠️ アクセシビリティ権限はキー入力を送れる強力な権限です。**信頼できる配布元から入手したアプリにのみ**許可してください。
...
- **ソースコードを共有**し、相手側で `xcodegen generate` → Xcode でビルドするのが最も安全です（Gatekeeper 警告なし）。
- ビルド済み `.app` を渡す場合、ad-hoc 署名のため相手の Mac で「開発元を確認できません」と警告が出ます。**右クリック →「開く」** で起動できます（正式に配るなら Apple Developer 署名＋公証が必要）。
- 相手の環境が **Apple Silicon Mac ＋ macOS 26 以降** であることを必ず確認してください。
- 他社製ディクテーションアプリの `.dmg` など、**第三者の市販アプリを同梱・再配布しないでください**。
...
## ⚖️ 法的ドキュメント（販売用テンプレート）

有料販売に向けた雛形を `legal/` に用意しています。**`【...】` を自分の情報に置き換え、公開前に専門家の確認**を推奨します。

- [プライバシーポリシー](legal/PRIVACY_POLICY.md)
- [利用規約（EULA）](legal/TERMS_OF_USE.md)
- [特定商取引法に基づく表記](legal/特定商取引法に基づく表記.md)

## 📝 ライセンス

[MIT License](LICENSE) で公開しています。自由に使用・改変・再配布できます（作者表示と免責の保持が条件）。
